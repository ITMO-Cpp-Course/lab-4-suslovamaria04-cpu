...
FileHandle::FileHandle(const std::string& path)
    : path_(path), file_(nullptr), is_open_(false) {
...
// ========== ВСПОМОГАТЕЛЬНЫЕ ==========
bool FileHandle::is_open() const noexcept { return is_open_; }

const std::string& FileHandle::get_path() const noexcept { return path_; }
