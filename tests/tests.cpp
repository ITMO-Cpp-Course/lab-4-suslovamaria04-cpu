...
TEST_CASE("ResourceManager persistence after shared_ptr destruction",
          "[ResourceManager]") {
  TempFile tf;
  auto& rm = ResourceManager::instance();
  {
...
TEST_CASE("ResourceManager multiple opens and cleanup", "[ResourceManager]") {
  ...
  for (int i = 0; i < 5; ++i) vec.push_back(rm.open_file(tf.path()));
...
TEST_CASE("Integration: multiple files", "[Integration]") {
  ...
  for (auto& tf : tfs) handles.push_back(rm.open_file(tf.path()));
...
TEST_CASE("Integration: exception on invalid path", "[Integration]") {
  REQUIRE_THROWS_AS(FileHandle("/nonexistent_dir_12345/file.txt"),
                    ResourceError);
}